"""
Main file for the project. This will create and run new experiments and load checkpoints
from wandb. Borrowed part of the code from David Charatan and wandb.
"""
...
from steerable_scene_generation.utils.cluster_utils import submit_slurm_job
...
def run_local(cfg: DictConfig):
    ...
    from steerable_scene_generation.utils.wandb_utils import (
        OfflineWandbLogger,
    ...
    if cfg.wandb.mode != "disabled":
        ...
        if "_on_compute_node" in cfg and cfg.cluster.is_compute_node_offline:
            ...
            # Use SpaceEfficientWandbLogger for reducing logged checkpoints on wandb.
            # logger_cls = SpaceEfficientWandbLogger

        is_offline = cfg.wandb.mode == "offline"
        logger = logger_cls(
            name=name,
            ...
            log_model=(
                "all" if not is_offline else False
            ),  # Required for custom wandb logger
...
def run(cfg: DictConfig):
    ...
    if not cfg.wandb.get("entity", None):
        raise ValueError(
            ...
            "line argument 'wandb.entity=[entity]' \n An entity is your wandb user name "
            "or group name. This is used for logging. If you don't have an wandb "
    ...
    # If resuming or loading a wandb ckpt and not on a compute node, download the
    # checkpoint.
    resume = cfg.get("resume", None)
    load = cfg.get("load", None)
    ...
    if "cluster" in cfg and not "_on_compute_node" in cfg:
        print(
            cyan(
                "Slurm detected, submitting to compute node instead of running "
                "locally..."
            )
        )
        run_slurm(cfg)
